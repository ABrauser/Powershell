Function Get-AzVMPublicIPAddress {
    <#
        ...
        .PARAMETER VMStatus
            If specified, the VM property from IncludeObjects will include data from Get-AzVM '-Status'
            Using this switch will trigger IncludeObjects
        .EXAMPLE
            Login-AzureRmAccount
            Get-AzVMPublicIP -ResourceGroupName 'my-resource-group'
        ...
        .EXAMPLE
            Login-AzureRmAccount
            Get-AzVMPublicIP -ResourceGroupName 'my-resource-group' -VMName VM-1* -IncludeObjects
                # ...
                # VMName   : VM-18
                # NICName  : VM-18-NIC
        ...
        .EXAMPLE
            $Details = Get-AzVMPublicIP -ResourceGroupName 'my-resource-group' -VMStatus
            $Details[0]
                # VMName   : VM-18
                # NICName  : VM-18-NIC
                # PublicIP : 23.96.19.71
                ...
                # NIC      : Microsoft.Azure.Commands.Network.Models.PSNetworkInterface
                # PIP      : Microsoft.Azure.Commands.Network.Models.PSPublicIpAddress
            # All sorts of data to explore in the VM property.  Output from Get-AzVM -Name <ThisVm> -Status
        ...
        .EXAMPLE
            Get-AzVMPublicIP -ResourceGroupName $ResourceGroup -VMName VM-18 -VMStatus |
    ...
    #>
        ...
        foreach($ResourceGroup in $ResourceGroupName)
        {
            ...
            Try
            {
                $AllVMs = @( Get-AzVM -ResourceGroupName $ResourceGroup -ErrorAction Stop )
            }
            Catch
            {
                Write-Error $_
            ...
            Foreach($nic in $nics)
            {
                ...
                foreach($VM in $VMs)
                {
                    if($VMStatus)
                    {
                        Try
                        {
                            $VMDetail = Get-AzVM -ResourceGroupName $ResourceGroup -Status -Name $VM.Name -ErrorAction stop
                        }
                        Catch
                        {
                            Write-Error $_
